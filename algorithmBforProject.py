...
def algorithmBforProject(twt):
    global modNo
    global dicLength
    global dic
    c = False
    dic = dict(zip(short, long))  # dictionary corresponding short and full, short is key
    dicLength = len(dic)
    ...
    for i in range(0, len(modTwt)):
        if modTwt[i] in dic.keys():
            modTwt[i] = dic[modTwt[i]]
            modNo = modNo+1
            c = True
...
for i in tweets:
    ...
    algorithmBforProject(i)
    l = l+1

print("__________________________________________________________________________________________________________________________________________________")
print("Changed to Full Form: " + str(modNo) + " Number(s) of Tweet(s) From")
print("Analyzing " + str(len(tweets)) + " Tweets Using")
print("Dictionary of " + str(dicLength) + " Entries")
print()
print("To Run The Better Algorithm B, It Took " + str((time.time() - start_time)) + " Seconds to ANALYZE " + str(len(tweets)) + " Tweets")
print("__________________________________________________________________________________________________________________________________________________")
